...
- **Spike first:** OpenCode is terminal- and TUI-oriented; confirm a **supported** non-interactive / headless / one-shot command in upstream docs or `--help` before a full adapter. If no such contract exists, keep this issue spike-only and stop.

## Vendor documentation (required)

OpenCode project (install, configuration, CLI surface): https://github.com/anomalyco/opencode
